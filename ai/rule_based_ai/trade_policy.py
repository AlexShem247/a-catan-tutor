...
class TradePolicy:
    ...
    def __init__(self, random_ai: RandomAI, etw_estimator, decision_config, use_strategic_move, planner_kwargs,
                 etw_kwargs, trade_risk_kwargs):
    ...
    def choose_trade_partner_with_explanation(
        ...
        if not self._use_strategic_move():
            ...
            explanation = self.explain_trade_partner_choice(player, game, selling, buying, available_players,
        ...
        explanation = self._build_trade_partner_explanation(player, sim_us, sim_game, selling, buying,
                                                            available_players, chosen_player, counter)
    ...
    def explain_trade_partner_choice(self, player: Player, game: Game, selling: ResourceCount, buying: ResourceCount,
        ...
        sim_game = make_sim_game_for_player(game, player)
        sim_us = sim_game.overlay.get_sim_player(player.player_number)
        return self._build_trade_partner_explanation(player, sim_us, sim_game, selling, buying, available_players,
                                                     chosen_player, counter)
    ...
    def respond_to_trade_with_explanation(
        ...
        if not self._use_strategic_move():
            ...
            explanation = self.explain_trade_response_choice(player, game, opponent, selling, buying, accepted, counter)
        ...
        accepted, counter = respond_to_trade_batna(
            ...
            **{
                key: value
                for key, value in self._trade_risk_kwargs().items() if key != "leader_penalty"
            },
        )
        explanation = self._build_trade_response_explanation(player, opponent, sim_us, opponent_sim, sim_game, selling,
                                                             buying, opponents, accepted, counter)
        return accepted, counter, explanation
    ...
    def explain_trade_response_choice(self, player: Player, game: Game, opponent: Player, selling: ResourceCount,
        ...
        opponent_sim = SimPlayerState(opponent, opponent=True)
        opponents = get_opponents(sim_game, player.player_number)
        return self._build_trade_response_explanation(player, opponent, sim_us, opponent_sim, sim_game, selling, buying,
                                                      opponents, accepted, counter)

    def _build_trade_partner_explanation(self, player: Player, sim_us: SimPlayerState, sim_game, selling: ResourceCount,
                                         buying: ResourceCount, available_players: list[tuple[Player,
                                                                                              ResourceCount | None]],
                                         chosen_player: Player, counter: ResourceCount | None) -> ActionExplanation:
        """Build the trade partner explanation."""
        batna_etw = self.etw_estimator.estimated_time_to_win(sim_us, sim_game, False,
        ...
        etw_by_player = {
            sim_player: self.etw_estimator.estimated_time_to_win(sim_player.copy(), sim_game, False,
                                                                 **self._etw_kwargs(include_player_trades=False))
        ...
        for opponent, offered_counter in available_players:
            ...
            etw_after = self.etw_estimator.estimated_time_to_win(sim_after, sim_game_after, False,
                                                                 **self._etw_kwargs(include_player_trades=False))
        ...
        chosen_candidate = next(
            (candidate for candidate in candidate_explanations
             ...
             and candidate.metadata.get("payment") == (counter if counter is not None else selling)), None)
    ...
    def _build_trade_response_explanation(self, player: Player, opponent: Player, sim_us: SimPlayerState,
                                          opponent_sim: SimPlayerState, sim_game, selling_to_us: ResourceCount,
                                          buying_from_us: ResourceCount, opponents: list[SimPlayerState],
                                          accepted: bool, counter: ResourceCount | None) -> ActionExplanation:
        """Build the trade response explanation."""
        etw_before = self.etw_estimator.estimated_time_to_win(sim_us, sim_game, False,
                                                              **self._etw_kwargs(include_player_trades=False))
        ...
        etw_after_offer = self.etw_estimator.estimated_time_to_win(offered_after, offered_game, False,
                                                                   **self._etw_kwargs(include_player_trades=False))
        ...
        delta_opp_offer = 0.0
        if self.decision_config.use_opponent_interference:
            opp_before = self.etw_estimator.estimated_time_to_win(opponent_sim, sim_game, False,
                                                                  **self._etw_kwargs(include_player_trades=False))
            ...
            opp_after = self.etw_estimator.estimated_time_to_win(opp_after_offer, opp_game, False,
                                                                 **self._etw_kwargs(include_player_trades=False))
        ...
        if accepted and counter is None:
            ...
            etw_after = self.etw_estimator.estimated_time_to_win(counter_after, counter_game, False,
                                                                 **self._etw_kwargs(include_player_trades=False))
